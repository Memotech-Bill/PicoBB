...
REM Works in BBCSDL (v0.24a or later) and BB4W (v6.12a or later).
...
ON ERROR ON ERROR OFF : PROCcleanup : IF ERR=17 CHAIN @lib$+"../examples/tools/touchide" ELSE PRINT 'REPORT$:END
...
result% = FN_sendtosocket(UDP_socket%, buffer%%, 1, broadcast%, port%)
REPEAT
result% = FN_recvfromsocket(UDP_socket%, buffer%%, buflen%, host%, discard%)
UNTIL result% = 1
hosts%(0) = broadcast%
