...
170 INPUT "Server address", server$, "Server port", port%
180 addr% = 0
190 SYS "ip4addr_aton", server$, ^addr% TO err%
200 IF err% <> 0 THEN
210 PRINT "Error ";err%;" in Server address"
220 END
230 ENDIF
240 port% = 256 * ?^port% + ?(^port + 1)
250 SYS "net_tcp_connect", addr%, port%, 5000 TO conn%
260 IF conn% < 0 THEN
270 PRINT "Error ";conn%;" connecting to server"
280 END
290 ENDIF
300 PRINT "Connected to server"
310 DIM data&(2047)
320 FOR i% = 1 TO 10
330 SYS "net_tcp_read", conn%, 2048, ^data&(0), 10000 TO err%
340 IF err% < 0 THEN
...
410 NEXT
420 PRINT
430 SYS "net_tcp_write", conn%, err%, ^data&(0), 10000 TO err%
440 IF err% < 0 THEN
