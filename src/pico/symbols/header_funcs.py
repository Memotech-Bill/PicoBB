...
class Header:
    ...
    def define (self, sLine):
        ...
        if ( value.isdecimal () ):
            ...
            value = self.defs[value]
        elif DEBUG:
            self.fOut.write (f'### Not defined: {value}\n')
    ...
    def expr_val (self, sVal):
        sVal = sVal.strip ()
        if sVal.isdecimal ():
            return int (sVal, 10)
        elif sVal.startswith ('!'):
            return not self.expr_val (sVal[1:])
        elif sVal.startswith ('defined'):
            return sVal[sVal.find ('(') + 1:sVal.find (')')] in self.defs
        elif sVal in self.defs:
            return self.expr (self.defs[sVal])
        elif sVal.startswith (')') and sVal.endswith (')'):
            return self.expr (sVal[1:-1])
        elif DEBUG:
            self.fOut.write (f'### Not defined: {sVal}\n')
        return False

    def expr_cond (self, sOp, sTerm):
        parts = sTerm.split (sOp, 1)
        parts[0] = self.expr_val (parts[0])
        parts[1] = self.expr_val (parts[1])
        if DEBUG:
            self.fOut.write (f'# sOp = {sOp}, parts = {parts}\n')
        if sOp == '==':
    ...
    def expr_term (self, sTerm):
        for sOp in ['==', '!=', '<=', '>=', '<', '>']:
            if sOp in sTerm:
                return self.expr_cond (sOp, sTerm)
        return self.expr_val (sTerm.strip ())

    def expr_and (self, sAnd):
        for sPart in sAnd.split ('&&'):
            if not self.expr_term (sPart):
                return False
        return True

    def expr (self, expn):
        expn = self.defs.get (expn, expn)
        if isinstance (expn, str):
            expn = expn.strip ()
            for sPart in expn.split ('||'):
                if self.expr_and (sPart):
                    return True
            return False
        return expn
    ...
    def do_if (self, sLine):
        sLine = Body (sLine)
        do = self.expr (sLine) and self.active[-1][0]
        self.active.append ([do, do])

    def do_elseif (self, sLine):
        sLine = Body (sLine)
        do = self.expr (sLine) and self.active[-2][0]
        self.active[-1][0] = do
        if ( do ):
