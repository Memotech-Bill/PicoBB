...
void dumpmem (const char *pMem, int nMem)
    {
    char sDump[17];
    while (nMem > 0)
        {
...
//      10 or 12 = 80-bit floating point
//           16 = structure (64-bit) 
//           24 = structure (32-bit)
...
static void *scanll (heapptr *base, signed char *edi)
{
    ...
    // printf ("scanll: base = %p, esi = %p, edi = %p\r\n", base, esi, edi);
    // dumpmem (esi, 16);
    // printf ("Test = %X\r\n", *((int *)0x20027310));
...
static unsigned char getype (char *ptr)
    ...
    while ( *ptr == 0 );
	char al = *(ptr - 0) ; // type character
	char ah = *(ptr - 1) ; // to check for %%
	if (al == '(')
	    {
		al = *(ptr - 1) ; // array type
		ah = *(ptr - 2) ;
...
//       10 or 12 = 80-bit floating point (a)
...
// If found, return pointer to variable (etc.) and set type as appropriate.
// Types are: 1, 4, 5, 8, 10, 12, 40 numeric
//            128, 130, 136 string
//            16/24, 80/88 structure, structure array
//            36, 100 function or procedure
//            65, 68, 72, 74, 76, 104, 200 whole array
